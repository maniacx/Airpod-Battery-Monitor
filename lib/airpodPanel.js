...
class AirpodPanel extends PanelMenu.Button {
    constructor(settings, extensionObj, airpods) {
        super(0.5, _('Bluetooth battery Indicator'));
        log('---- aipdpanel');
...
const DeviceItem = GObject.registerClass({
    ...
    constructor(settings, extensionObj, path, model, data, airpodLength) {
        ...
        if (this._multipleInfo) {
            ...
            const leftBin = new St.Bin({style_class: 'abm-panel-space-bin', x_expand: true});
            const rightBox =  new St.BoxLayout({x_align: Clutter.ActorAlign.CENTER, y_align: Clutter.ActorAlign.CENTER});
            const rightBin = new St.Bin({style_class: 'abm-panel-space-bin', x_expand: true});
            const caseBox =  new St.BoxLayout({x_align: Clutter.ActorAlign.END, y_align: Clutter.ActorAlign.CENTER});
